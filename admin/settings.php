...
function wp_floating_cta_settings_page(): void {
    ...
    <div class="wrap">
        ...
        <style>
        /* ── 管理画面レイアウト ── */
        .fcta-layout { display:flex; gap:24px; align-items:flex-start; }
        .fcta-form-col { flex:1 1 0; min-width:0; }
        .fcta-preview-col {
            width:340px; flex-shrink:0;
            position:sticky; top:32px;
        }
        @media(max-width:900px){
            .fcta-layout { flex-direction:column; }
            .fcta-preview-col { width:100%; position:static; }
        }
        ...
        .fcta-section h2 { margin-top:0; padding-bottom:10px; border-bottom:1px solid #eee; font-size:15px; }

        /* ── プレビュー ── */
        .fcta-preview-col .fcta-section { position:sticky; top:32px; }
        #fcta-preview-frame {
        ...
        /* プレビューウィジェット本体 — #wp-floating-cta と同じ見た目、ただし static */
        #fcta-preview-widget {
        ...
        #fcta-preview-widget.fcta-fullwidth {
            max-width:100%; border-radius:0;
        }
        /* アニメーション類は floating-cta.css が担う */
        </style>

        <form method="post" action="options.php">
            <?php settings_fields( 'wp_floating_cta_group' ); ?>

            <div class="fcta-layout">

                <!-- ===== 左カラム: 設定フォーム ===== -->
                <div class="fcta-form-col">
                    ...
                    <div class="fcta-section">
                        ...
                        <div id="fcta-preview-frame">
                            <div id="fcta-preview-widget"
                                 ...
                                 style="padding:<?php echo (int) $s['bg_padding_v']; ?>px <?php echo (int) $s['bg_padding_h']; ?>px;">

                                <?php if ( ! empty( $s['show_close'] ) ) : ?>
                                <button type="button" class="fcta-close" style="display:block;">&#x2715;</button>
                                <?php else : ?>
                                <button type="button" class="fcta-close" style="display:none;">&#x2715;</button>
                                <?php endif; ?>

                                <p class="fcta-micro fcta-micro-top" id="fcta-prev-micro-top"
                                   style="color:<?php echo esc_attr( $s['micro_top_color'] ); ?>;font-size:<?php echo (int) $s['micro_top_size']; ?>px;<?php echo empty( trim( $s['micro_copy_top'] ) ) ? 'display:none;' : ''; ?>">
                                    <?php echo esc_html( $s['micro_copy_top'] ); ?>
                                </p>

                                <span id="fcta-prev-btn"
                                      class="fcta-btn<?php echo esc_attr( ( $btn_anim = ( $s['button_animation'] !== 'none' ) ? ' fcta-btn-' . $s['button_animation'] : '' ) ); ?>"
                                ...
                                <p class="fcta-micro fcta-micro-bottom" id="fcta-prev-micro-bot"
                                   style="color:<?php echo esc_attr( $s['micro_bottom_color'] ); ?>;font-size:<?php echo (int) $s['micro_bottom_size']; ?>px;<?php echo empty( trim( $s['micro_copy_bottom'] ) ) ? 'display:none;' : ''; ?>">
                                    <?php echo esc_html( $s['micro_copy_bottom'] ); ?>
                                </p>
                            </div>
                        </div>
                        <p class="description" style="margin-top:8px;font-size:11px;">※ プレビューはイメージです。実際の表示はサイトのフォントやCSSに依存します。</p>
                    </div>
                </div><!-- /.fcta-preview-col -->
    ...
    (function(){
        ...
        if(!chk || !sec) return;
        function toggle(){
            sec.style.opacity = chk.checked ? '.4' : '1';
            sec.style.pointerEvents = chk.checked ? 'none' : '';
        }
